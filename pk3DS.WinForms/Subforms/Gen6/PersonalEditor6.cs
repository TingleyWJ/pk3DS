    ...
    */
	...
	private void B_Export_Calc(object sender, EventArgs e)
	{
		...
		for (int i = 1; i < CB_Species.Items.Count; i++)
		{
			...
			{
				Types = [(string)CB_Type1.SelectedItem, (string)CB_Type2.SelectedItem],
				Abilities = [(string)CB_Ability1.SelectedItem, (string)CB_Ability2.SelectedItem, (string)CB_Ability3.SelectedItem]
			};

			if (CB_Type1.SelectedItem == CB_Type2.SelectedItem)
				ep.Types = [(string)CB_Type1.SelectedItem];
	...
	private void B_Export_Site(object sender, EventArgs e)
	{
		...
		Dictionary<string, ExportPokemonSite> pkmDict = [];
