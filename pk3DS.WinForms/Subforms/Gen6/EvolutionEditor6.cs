...
public partial class EvolutionEditor6 : Form
{
	...
	private void B_Export_Site(object sender, EventArgs e)
	{
		...
		Dictionary<string, ExportEvoSite> evoDict = [];
		...
		for (int i = 1; i < CB_Species.Items.Count; i++)
		{
			...
			for (int j = 0; j < 8; j++)
			{
				...
				if (AXHelpers.BetterEvoMethodDesc.TryGetValue(methodIdx, out string paramReplaceStr))
					method = paramReplaceStr.Replace("PARAM", pb[j].Text);
